...
describe("runtime: generate (default)", () => {
  ...
  it("inlines the forge runtime so the SDK never references this package", async () => {
    const { paths, get } = await generate({
      input: fixture,
      transports: { forge: { product: "jira" } },
    });
    // Inlined generic forge + product-bound wrapper importing the sibling. The
    // wrapper's only package mention is the "Generated by" header, not an import.
    expect(paths).toContain("transports/_forge.ts");
    const wrapper = get("transports/forge.ts")!;
    expect(wrapper).toContain('import { forgeJira } from "./_forge"');
    expect(wrapper).not.toContain("@narthia/openapi-sdk-generator/transports");
    // The copied runtime imports @forge/api directly; its client import is rewritten.
    const inlined = get("transports/_forge.ts")!;
    expect(inlined).toContain('import api, { assumeTrustedRoute } from "@forge/api"');
