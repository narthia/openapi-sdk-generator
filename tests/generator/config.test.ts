...
describe("runCli with a config file", () => {
  ...
  it("generates a multi-input tree from a config with `inputs`", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        output: outDir,
        inputs: {
          catalog: { input: fixture, name: "createCatalog" },
  ...
  it("lets a shared flag override the multi-input config", async () => {
    ...
    await writeFile(
      cfg,
      JSON.stringify({ output: outDir, inputs: { catalog: { input: fixture } } })
    );
    ...
    const config = await readFile(join(outDir, "catalog", "config.ts"), "utf8");
    expect(config).toContain('from "@narthia/openapi-sdk-generator/client"');
  });
});

describe("header flag and config", () => {
  const dirs: string[] = [];
  const makeDir = async () => {
    const dir = await mkdtemp(join(tmpdir(), "narthia-hdr-cfg-"));
    dirs.push(dir);
    return dir;
  };
  afterEach(async () => {
  ...
  const HEADER = "// Generated by @narthia/openapi-sdk-generator";
  ...
  it("emits the header by default", async () => {
    ...
    expect(await readFile(join(outDir, "index.ts"), "utf8")).toContain(HEADER);
  });

  it("omits it with --no-header", async () => {
    const dir = await makeDir();
    const outDir = join(dir, "sdk");
    const { io } = captureIo();
    expect(await runCli(["-i", fixture, "-o", outDir, "--no-header"], io)).toBe(0);
    expect(await readFile(join(outDir, "index.ts"), "utf8")).not.toContain(HEADER);
    // The inlined runtime takes a different emit path, so check it too.
    expect(await readFile(join(outDir, "client", "client.ts"), "utf8")).not.toContain(HEADER);
  });

  // Guards the CLI's explicit shared-option allow-list: a `header` in the config
  // file is dropped unless it is forwarded there.
  it("honours `header: false` from the config file", async () => {
    const dir = await makeDir();
    const outDir = join(dir, "sdk");
    const cfg = join(dir, "sdk.config.json");
    await writeFile(cfg, JSON.stringify({ input: fixture, output: outDir, header: false }));
  ...
  it("lets --no-header override `header: true` from the config file", async () => {
    const dir = await makeDir();
    const outDir = join(dir, "sdk");
    const cfg = join(dir, "sdk.config.json");
    await writeFile(cfg, JSON.stringify({ input: fixture, output: outDir, header: true }));

    const { io } = captureIo();
    expect(await runCli(["-c", cfg, "--no-header"], io)).toBe(0);
    expect(await readFile(join(outDir, "index.ts"), "utf8")).not.toContain(HEADER);
  });
});
