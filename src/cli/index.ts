...
import type { CleanMode } from "../generator/clean.ts";
import type { AuthOption, GenerateOptions, TransportsOption } from "../generator/generate.ts";
import type { RuntimeMode } from "./../generator/emit/ts-writer.ts";
...
Options:
  ...
  -n, --name <name>        Name of the generated SDK factory (default: createSdk)
      ...
      --forge-product <p>  Atlassian product for the forge transport: jira | confluence | bitbucket
      --forge-as <who>     Forge default identity: app | user (default: app)
      --clean <mode>       Output cleaning: all (empty the dir) | generated (only files this
                           generator emitted) | none (keep everything) (default: all)
      --no-header          Omit the "Generated by" header comment from emitted files
                           (cannot be combined with --clean generated)
      --normalize-version  Strip a -SNAPSHOT-<sha> build id from the documented API version
  -h, --help               Show this help
  -v, --version            Print the version

  * input and output may come from a config file instead of flags; flags override the config.

Auth (applies to the http transport; when omitted, schemes are derived from the spec's securitySchemes):
...
Examples:
  ...
  openapi-sdk-generator -i ./jira.json -o ./sdk --transports forge --forge-product jira`;
...
export async function runCli(
  ...
  try {
    ({ values } = parseArgs({
      ...
      options: {
        ...
        "forge-product": { type: "string" },
        "forge-as": { type: "string" },
        clean: { type: "string" },
        "no-header": { type: "boolean" },
        "normalize-version": { type: "boolean" },
  ...
  const cleanFlag = values.clean;
  if (cleanFlag !== undefined && !["all", "generated", "none"].includes(cleanFlag)) {
    io.err(`Error: --clean must be one of "all", "generated", or "none" (got "${cleanFlag}").`);
    return 1;
  }
  const CLEAN_MODES: Record<string, CleanMode> = { all: true, generated: "generated", none: false };
  const clean = cleanFlag ? CLEAN_MODES[cleanFlag] : undefined;
  ...
  const transportsResult = buildTransportsOption(values, authResult.auth);
  if (transportsResult.error) {
    io.err(`Error: ${transportsResult.error}`);
    return 1;
  }
  ...
  const shared = {
    ...
    transports: transportsResult.transports ?? fileConfig.transports,
    clean: clean ?? fileConfig.clean,
    header: values["no-header"] === true ? false : fileConfig.header,
    normalizeVersion: values["normalize-version"] === true ? true : fileConfig.normalizeVersion,
  };

  // Multi-input is config-file only: per-input transports/name/collisionCase and
  // the top-level shared `transports` come from the config; shared flags above
  // (including `--transports`/`--auth-*`/`--forge-*`) still override the shared
  // level. Per-single-input flags (--input, --name, --collision-case) don't apply here.
  const multiInputs =
...
/**
 * Build a {@link TransportsOption} from CLI flags. Returns `{ transports: undefined }`
 * when no transport-related flag is given (so the config file / generator default
 * applies), or `{ error }` on invalid input.
 *
 * `--transports` lists which to enable (`http`, `forge`); when omitted it is
 * inferred from the other flags (`http` when `--auth-*` is present, `forge` when
 * `--forge-*` is present). Http carries the resolved `auth`; forge requires
 * `--forge-product`.
 */
export function buildTransportsOption(
  values: AuthFlags,
  auth: AuthOption | undefined
): { transports?: TransportsOption; error?: string } {
  const listFlag = values.transports;
  const forgeProduct = str(values["forge-product"]);
  const forgeAs = str(values["forge-as"]);

  const explicit =
    typeof listFlag === "string"
      ? listFlag
          .split(",")
  ...
  // Nothing transport-related on the command line: defer to config/defaults.
  if (explicit === undefined && auth === undefined && !forgeProduct && !forgeAs) {
    return { transports: undefined };
  }

  const names =
    explicit ??
    [forgeProduct || forgeAs ? "forge" : undefined, auth ? "http" : undefined].filter(
      (n): n is string => n !== undefined
    );

  const transports: TransportsOption = {};
  for (const name of names) {
    if (name === "http") {
      transports.http = { auth };
    } else if (name === "forge") {
      if (!forgeProduct) {
        return { error: "--forge-product is required when generating a forge transport." };
      }
      if (!["jira", "confluence", "bitbucket"].includes(forgeProduct)) {
        return {
          error: `--forge-product must be one of "jira", "confluence", or "bitbucket" (got "${forgeProduct}").`,
        };
      }
      if (forgeAs !== undefined && forgeAs !== "app" && forgeAs !== "user") {
        return { error: `--forge-as must be one of "app" or "user" (got "${forgeAs}").` };
      }
      transports.forge = {
        product: forgeProduct as "jira" | "confluence" | "bitbucket",
        as: forgeAs as "app" | "user" | undefined,
      };
    } else {
      return { error: `--transports values must be "http" or "forge" (got "${name}").` };
    }
  }

  if (!transports.http && !transports.forge) return { transports: undefined };
  return { transports };
}
